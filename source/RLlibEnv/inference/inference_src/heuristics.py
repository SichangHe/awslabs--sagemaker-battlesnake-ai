...
class Heuristics:
    ...
    def positive_heuristics(func):
        def positive_heuristics_func(self, *args, **kwargs):
            action = func(self, *args, **kwargs)
            assert np.sum(action) in [
                1,
                4,
            ], "Only 1 true reward is allowed or all True are allowed"
            return action
    ...
    def _convert_state_into_json(
        self, map_size, state, snake_list, snake_id, turn_count, health
    ):
        """
    ...
    def get_action_masks_from_functions(
        self, state, snake_id, turn_count, health, env, functions
    ):
        """ """
        snake_list = self._make_snake_lists(env)
        map_size = env.map_size
        json = self._convert_state_into_json(
            map_size, state, snake_list, snake_id, turn_count, health
        )
