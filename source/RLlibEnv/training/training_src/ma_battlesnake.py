...
class MultiAgentBattlesnake(MultiAgentEnv):
    ...
    def __init__(self, num_agents, map_height, heuristics, rewards=SimpleRewards()):
        ...
        gym_observation_space = gym.spaces.Box(
            low=-1.0,
            high=5.0,
            shape=(self.observation_height, self.observation_height, 6),
            dtype=np.float32,
        )

        self.observation_space = gym.spaces.Dict(
            {
                "action_mask": gym.spaces.Box(0, 1, shape=(4,), dtype=np.float32),
        ...
        for i in range(self.num_agents):
            ...
            if len(self.heuristics) > 0:
                ...
                mask = self.battlesnake_heuristics.get_action_masks_from_functions(
                    obs_i, i, 0, health, self.env, functions=self.heuristics_list
                )
            else:
