...
class JoinManager:
    ...
    def __init__(
        ...
        if (
            input_obs_data_s3_path
            and input_obs_data_s3_path != "local-join-does-not-apply"
        ):
            self._create_obs_table_if_not_exist()
        # create reward table if not exists
        if (
            input_reward_data_s3_path
            and input_reward_data_s3_path != "local-join-does-not-apply"
        ):
    ...
    def _create_athena_s3_bucket_if_not_exist(self):
        ...
        try:
            ...
            logger.info(
                "Successfully create S3 bucket '{}' for athena queries".format(
                    s3_bucket_name
                )
            )
    ...
    def _create_rewards_table_if_not_exist(self):
        ...
        logger.debug(
            f"Successfully create rewards table '{self.rewards_table}' for query"
        )
        ...
        logger.debug(
            f"Successfully update s3 location of rewards table '{self.rewards_table}'"
        )
    ...
    def _add_time_partitions(self, start_time, end_time):
        ...
        self.wait_query_to_finish(query_id)
        logger.debug(
            f"Successfully add partitions to table {self.obs_table_partitioned}"
        )

    def _get_join_query_string(
        self, ratio=0.8, train_data=True, start_time=None, end_time=None
    ):
    ...
    def _start_query(self, query_string, s3_output_path):
        ...
        try:
            ...
            message = e.response["Error"]["Message"]
            raise RuntimeError(
                "Failed to submit athena query with error {}: {}".format(
                    error_code, message
                )
            )
        return query_id
    ...
    def start_join(self, ratio=0.8, wait=True):
        ...
        self.join_db_client.update_join_job_join_query_ids(
            self.experiment_id,
            self.join_job_id,
            [join_query_id_for_train, join_query_id_for_eval],
        )
    ...
    def _upload_data_buffer_as_joined_data_format(
        self, data_buffer, s3_bucket, s3_prefix
    ):
        ...
        try:
            ...
            s3_client.put_object(
                Body=body, Bucket=s3_bucket, Key=joined_data_s3_file_key
            )
        except ClientError as e:
            error_code = e.response["Error"]["Code"]
            message = e.response["Error"]["Message"]
            logger.error(
                "Failed to upload local joined data with error {}: {}".format(
                    error_code, message
                )
            )
            return None
