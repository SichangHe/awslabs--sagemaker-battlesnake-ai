...
class ExperimentManagerSyncThread(Thread):
    ...
    def _update_experiment_db_training_workflow_metadata(
        self, training_workflow_metadata
    ):
        """
        ...
        next_model_to_train_id = training_workflow_metadata.get(
        ...
        if training_state is None:
            ...
            if training_job_record is None:
                ...
                if train_state_from_modeldb is not None:
                    new_training_state = TRAINING_JOB_STATUS_MAP[
                        train_state_from_modeldb
                    ]
                else:
        ...
        try:
            ...
            if "ConditionalCheckFailedException" in str(e):
                ...
                return
            logger.error(
                "Failed to update ExperimentDb with latest information: " + str(e)
            )
        ...
        self.experiment_manager.experiment_record._training_state = (
            training_workflow_metadata["training_state"]
        )

    def _update_experiment_db_evaluation_workflow_metadata(
        self, evaluation_workflow_metadata
    ):
        """
        ...
        next_evaluation_job_id = evaluation_workflow_metadata.get(
        ...
        if evaluation_state is not None and evaluation_state.endswith("ING"):
            ...
            if evaluation_job_record is not None:
                ...
                if eval_state_from_modeldb is not None:
                    evaluation_state = EVALUATION_JOB_STATUS_MAP[
                        eval_state_from_modeldb
                    ]

                self.experiment_manager.experiment_record._evaluation_state = (
                    evaluation_state
                )
                ...
                if evaluation_state == EvaluationState.EVALUATED:
                    ...
                    self.experiment_manager.experiment_record._next_evaluation_job_id = (
                        None
                    )
    ...
    def _update_experiment_db_hosting_workflow_metadata(
        self, hosting_workflow_metadata
    ):
        ...
        next_model_to_host_id = hosting_workflow_metadata.get(
            "next_model_to_host_id", None
        )
        last_hosted_model_id = hosting_workflow_metadata.get(
        ...
        if hosting_state is not None and hosting_state.endswith("ING"):
            if (hosting_endpoint is None) or (
                not self.experiment_manager.soft_deployment
            ):
                ...
                hosting_state = HOSTING_ENDPOINT_STATUS_MAP[
                    sm_endpoint_info.get("EndpointStatus")
                ]
                ...
                if hosting_state == HostingState.DEPLOYED:
                    ...
                    self.experiment_manager.experiment_record._next_model_to_host_id = (
                        None
                    )
                    ...
                    self._update_metrics_from_latest_hosting_update(
                        next_model_to_host_id
                    )
            else:
                ...
                if self.experiment_manager.soft_deployment:
                    ...
                    while model_id != next_model_to_host_id:
                        ...
                        model_id = predictor.get_hosted_model_id()
                        num_retries += 1
                        if (not self.experiment_manager.local_mode) or (
                            num_retries >= 5
                        ):
                            break
                        time.sleep(1)
                    ...
                    self.experiment_manager.experiment_record._hosting_state = (
                        hosting_state
                    )
    ...
    def _update_experiment_db_joining_workflow_metadata(
        self, joining_workflow_metadata
    ):
        ...
        if joining_state is not None and joining_state.endswith("ING"):
            ...
            if join_job_record is not None:
                ...
                if joining_state == JoiningState.SUCCEEDED:
                    self.experiment_manager.experiment_record._last_joined_job_id = (
                        next_join_job_id
                    )
    ...
    def _update_metrics_from_latest_eval_job(self, latest_evaluation_job_id):
        ...
        try:
            ...
            if last_trained_model_id in latest_evaluation_job_id:
                ...
                eval_score = self.get_latest_eval_score_for_model_id(
    ...
    def sync_experiment_state_with_ddb(self):
        ...
        self.experiment_manager.experiment_record = (
            ExperimentRecord.load_from_ddb_record(record)
        )
        ...
        time.sleep(1)
        self._update_experiment_db_training_workflow_metadata(
            training_workflow_metadata
        )
        ...
        time.sleep(1)
        self._update_experiment_db_evaluation_workflow_metadata(
        ...
        if next_join_job_id is not None and joining_state.endswith("ING"):
            if self.experiment_manager.next_join_job is not None:
                ...
                if (
                    self.join_db_client.get_join_job_record(
                        self.experiment_id, next_join_job_id
                    )
                    is not None
                ):
...
class ExperimentManager:
    ...
    def __init__(
        ...
        # load configs
        self.config = config
        self.image = self.config.get("image", None).replace(
        ...
        self.cw_logger = CloudWatchLogger(
            self.boto_session.client("cloudwatch"), self._region_name
        )
        ...
        try:
            ...
            self.experiment_record = ExperimentRecord.load_from_ddb_record(
                experiment_record
            )
        except Exception as e:
            logger.error("Unhandled Exception! " + str(e))
            raise UnhandledWorkflowException(
                "Something went wrong while creating a new experiment"
            )
    ...
    def _update_instance_type_for_local_mode(self):
        ...
        self.config["resource"]["private_resource"]["hosting_fleet"][
            "instance_type"
        ] = "local"
        self.config["resource"]["private_resource"]["training_fleet"][
            "instance_type"
        ] = "local"
        self.config["resource"]["private_resource"]["evaluation_fleet"][
            "instance_type"
        ] = "local"
    ...
    def _write_manifest_to_s3(self, manifest_file):
        ...
        manifest_s3_file_key = (
            f"{self.experiment_id}/manifest_files/manifest-{timstamp}"
        )
        body = b""
        body += str(json.dumps(manifest_file, sort_keys=True, indent=4)).encode("utf_8")
        try:
            s3_client.put_object(
                Body=body, Bucket=manifest_bucket_name, Key=manifest_s3_file_key
            )
            ...
            raise RuntimeError(
                "Failed to upload manifest data with error {}: {}".format(
                    error_code, message
                )
            )
        ...
        logger.info(
            f"Successfully upload manifest file to s3 bucket path `{manifest_file_path}'"
        )
    ...
    def _generate_manifest(self, input_data_path_list):
        ...
        # ]
        manifest = []
        shared_prefix, key_path_list = self._get_prefix_and_relative_path(
            input_data_path_list
        )
        logger.info(
            f"Generating manifest file with shared prefix '{shared_prefix}/' ..."
        )
    ...
    def last_hosted_model_id(self):
        if self.experiment_record._last_hosted_model_id is None:
            logger.warning(
                "No model has been hosted. Please deploy a model and check later."
            )
    ...
    def deploy_model(self, model_id, wait=True, **kwargs):
        ...
        if (
            ...
            logger.info(
                f"Model {model_id} is already being hosted. No deployment needed."
            )
            return
        ...
        if self.experiment_record._hosting_endpoint is None:
            ...
            if self.local_mode:
                ...
                logger.info(
                    "No hosting endpoint found, creating a new hosting endpoint."
                )
            ...
            if self.experiment_record._hosting_state.endswith("ING"):
                logger.warning(
                    "Some deployment request is in progress, canceled this one"
                )
                return
            elif self.experiment_record._hosting_state.endswith("ED"):
                self._update_model_in_endpoint(
                    self.soft_deployment, model_id, wait=wait
                )
        ...
        if self.local_mode or wait:
            ...
            while not deployed_state:
                ...
                self._sync_experiment_state_with_ddb()
                logger.debug(
                    "Waiting for experiment table hosting status to be updated..."
                )
    ...
    def ingest_rewards(self, rewards_buffer):
        ...
        rewards_bucket_name = self.resource_manager._create_s3_bucket_if_not_exist(
            "sagemaker"
        )
        timstamp = str(int(time.time()))
        ...
        try:
            ...
            message = e.response["Error"]["Message"]
            raise RuntimeError(
                "Failed to upload rewards data with error {}: {}".format(
                    error_code, message
                )
            )
        ...
        logger.info(
            f"Successfully upload reward files to s3 bucket path {rewards_file_path}"
        )
    ...
    def ingest_joined_data(self, joined_data_buffer, ratio=0.8):
        ...
        next_join_job_id = JoinManager.name_next_join_job(
            experiment_id=self.experiment_id
        )
        self.exp_db_client.update_experiment_next_join_job_id(
            self.experiment_id, next_join_job_id
        ...
        self.exp_db_client.update_experiment_joining_state(
            self.experiment_id, JoiningState.PENDING
        )
        ...
        self.next_join_job.start_dummy_join(
            joined_data_buffer=joined_data_buffer, ratio=ratio
        )
    ...
    def join(self, rewards_s3_path, obs_time_window=None, ratio=0.8, wait=True):
        ...
        if self.local_mode or wait:
            ...
            while not succeeded_state:
                ...
                self._sync_experiment_state_with_ddb()
                logger.debug(
                    "Waiting for experiment table joining status to be updated..."
                )
                time.sleep(2 * (2**num_retries))
                succeeded_state = (
                    self.experiment_record._joining_state == JoiningState.SUCCEEDED
    ...
    def initialize_first_model(self, wait=True, input_data_s3_prefix=None):
        ...
        if (
            ...
            next_model_to_train_id = ModelManager.name_next_model(
                experiment_id=self.experiment_id
            )
        ...
        if self.local_mode or wait:
            trained_state = (
                ...
                and self.experiment_record._last_trained_model_id
                == next_model_to_train_id
            ...
            while not trained_state:
                ...
                self._sync_experiment_state_with_ddb()
                logger.debug(
                    "Waiting for experiment table training status to be updated..."
                )
                time.sleep(2 * (2**num_retries))
                trained_state = (
                    self.experiment_record._training_state == TrainingState.TRAINED
    ...
    def train_next_model(
        self, wait=True, input_data_s3_prefix=None, input_model_id=None
    ):
        """
        ...
        if (
            input_model_id is None
            and self.experiment_record._last_trained_model_id is not None
        ):
            logger.info(
        ...
        if (
            ...
            logger.info(
                f"Starting training job for ModelId '{next_model_to_train_id}''"
            )
    ...
    def evaluate_model(
        self, input_data_s3_prefix=None, evaluate_model_id=None, wait=True
    ):
        """
        ...
        if evaluate_model_id is None:
            ...
            if not self._check_if_model_ready(evaluate_model_id):
                logger.error(
                    f"ModelId {evaluate_model_id} is not ready for evaluation."
                )
        ...
        if not evaluate_model_id:
            ...
            raise InvalidUsageException(
                "Please provide a valid ModelId to be evaluated"
            )
        ...
        if self.local_mode or wait:
            evaluated_state = (
                ...
                and self.experiment_record._last_evaluation_job_id
                == next_evaluation_job_id
            ...
            while not evaluated_state:
                ...
                self._sync_experiment_state_with_ddb()
                logger.debug(
                    "Waiting for experiment table evaluation status to be updated..."
                )
                time.sleep(2 * (2**num_retries))
                evaluated_state = (
                    self.experiment_record._evaluation_state
                ...
                if (
                    ...
                    or self.experiment_record._evaluation_state
    ...
    def _close_existing_containers(self):
        ...
        running_containers = [
            i for i in client.containers.list() if self.image in i.image.tags
        ]
