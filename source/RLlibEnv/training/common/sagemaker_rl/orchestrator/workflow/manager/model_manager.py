...
class ModelManager:
    ...
    def _fit_first_model(
        ...
        if manifest_file_path:
            ...
            self.rl_estimator.fit(
                job_name=self.model_id, inputs=input_data, wait=wait, logs=logs
            )
        else:
            self.rl_estimator.fit(
    ...
    def fit(
        ...
        if input_model_id is None:
            ...
            self.rl_estimator.fit(
                job_name=self.model_id, inputs=inputs, wait=wait, logs=logs
            )
            ...
            if self.local_mode:
                ...
                while eval_score == "n.a." and attempts < 4:
                    try:
                        ...
                        eval_score = str(
                            metric_df[metric_df["metric_name"] == "average_loss"][
                                "value"
                            ][0]
                        )
                    except Exception:
