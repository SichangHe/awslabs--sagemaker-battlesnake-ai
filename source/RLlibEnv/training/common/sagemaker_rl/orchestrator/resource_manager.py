...
class ResourceManager(object):
    ...
    def __init__(self, resource_config, boto_session=None):
        ...
        self.shared_resource_stack_name = self._resource_config.get(
            "shared_resource"
        ).get("resources_cf_stack_name")
        ...
        self.evaluation_fleet_config = self._resource_config.get(
            "private_resource"
        ).get("evaluation_fleet")
    ...
    def create_shared_resource_if_not_exist(self):
        ...
        experiment_db_session = self.boto_session.resource("dynamodb").Table(
            self.exp_db_table_name
        )
        self.exp_db_client = ExperimentDbClient(experiment_db_session)

        join_db_session = self.boto_session.resource("dynamodb").Table(
            self.join_db_table_name
        )
        self.join_db_client = JoinDbClient(join_db_session)

        model_db_session = self.boto_session.resource("dynamodb").Table(
            self.model_db_table_name
        )
    ...
    def _usable_shared_cf_stack_exists(self):
        ...
        if stack_status in ["UPDATE_COMPLETE", "CREATE_COMPLETE"]:
            ...
            logger.error(
                ...
                " Please wait for the delete to complete".format(
            ...
            logger.info(
                "Stack with name {} exists in {} state".format(stack_name, stack_status)
            )
            ...
            parameters = [
                {
                    ...
                    "ParameterValue": self._get_iam_role_property(
                        "role_name", "role_for_cl"
                    ),
    ...
    def _wait_for_cf_stack_create_to_complete(self):
        ...
        try:
            ...
            logger.error(e)
            logger.error(
                "Failed to Create Stack with name {} ".format(
                    self.shared_resource_stack_name
                )
            )
            raise Exception(
    ...
    def _wait_for_cf_stack_update_to_complete(self):
        ...
        try:
            ...
            logger.error(e)
            logger.error(
                "Failed to use Stack with name {} ".format(
                    self.shared_resource_stack_name
                )
            )
            raise Exception(
    ...
    def _parse_template(self):
        ...
        """
        with open(
            "./common/sagemaker_rl/orchestrator/cloudformation.yaml"
        ) as template_fileobj:
            ...
            json_params = [
                {
                    ...
                    "ParameterValue": self._get_resource_property(
                        parameter_prefix, "table_name"
                    ),
                    "UsePreviousValue": True,
                    "ResolvedValue": "string",
                },
    ...
    def _get_experiment_db_property(self, property_name, default_value=None):
        ...
        """
        experiment_db_config = self._resource_config.get("shared_resource").get(
            "experiment_db"
        )
    ...
    def _get_cf_output_by_key(self, output_key):
        ...
        raise Exception(
            "Cannot find an Output with OutputKey {} in Shared CF Stack".format(
                output_key
            )
        )
    ...
    def _init_firehose_from_config(
        ...
        if stream_name in exist_delivery_streams:
            logger.warning(
                f"Delivery stream {stream_name} already exist. "
    ...
    def create_firehose_stream_if_not_exists(self, stream_name, s3_prefix):
        ...
        self._init_firehose_from_config(
            stream_name, self.firehose_s3_bucket_name, s3_prefix
        )
