...
class SageClusterCommunicator:
    def __init__(self):
        ...
        self.aws_region = (
            boto3.Session().region_name if aws_region is None else aws_region
        )
    ...
    def _find_s3_output_path(self):
        ...
        if not module_dir_s3_path.startswith("s3://"):
            raise ValueError(
                'Unexpected format for module_dir_s3_path.  Expected "s3://...'
            )
    ...
    def create_s3_signal(self, signal):
        ...
        s3_client.upload_fileobj(
            io.BytesIO(b""), self.s3_bucket, self._get_s3_key(signal)
        )
    ...
    def write_host_config(self, ip, host_name):
        ...
        s3_client.upload_fileobj(
            file_handle, self.s3_bucket, self._get_s3_key(self.ip_key)
        )
        s3_client.upload_fileobj(
    ...
    def get_master_config(self):
        ...
        try:
            s3_client.download_file(
                self.s3_bucket, self._get_s3_key(self.ip_key), "ip.json"
            )
            ...
            raise RuntimeError(
                "Cannot fetch IP of redis server running in SageMaker:", e
            )
    ...
    def _wait_for_ip_upload(self, timeout=600):
        ...
        while True:
            ...
            if "Contents" not in response:
                ...
                if time_elapsed >= timeout:
                    raise RuntimeError(
                        "Cannot retrieve IP of redis server running in SageMaker"
                    )
            else:
                return
