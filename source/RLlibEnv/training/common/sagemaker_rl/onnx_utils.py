...
def get_correct_outputs(model):
    ...
    if "_continuousppohead" in graph_name:
        ...
        raise Exception(
            "Can't determine the RL Agent used from the ONNX graph provided."
        )
...
def make_output(node_name, shape):
    ...
    return helper.make_tensor_value_info(
        name=node_name, elem_type=TensorProto.FLOAT, shape=shape
    )
...
def ppo_discrete_outputs(model):
    ...
    value_head = make_output(value_head_name, shape=(1,))
    policy_head_name = (
        "generalmodel0_singlemodel1_scaledgradhead0_discreteppohead0_softmax0"
    )
