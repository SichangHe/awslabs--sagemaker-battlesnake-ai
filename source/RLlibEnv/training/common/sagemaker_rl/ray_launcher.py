...
class SageMakerRayLauncher(object):
    ...
    def __init__(self):
        ...
        self.is_master_node = (
            self.hosts_info[0] == self.host_name
            and self.cluster_type == Cluster.Primary
        )
    ...
    def get_all_host_names(self):
        ...
        for i in range(self.num_instances_secondary_cluster):
            ...
            all_workers_host_names.append(
                "%s:algo-%s" % (Cluster.Secondary.value, i + 1)
            )
    ...
    def ray_init_config(self):
        ...
        if self.is_master_node:
            ...
            master_ip, master_hostname = (
                self.sage_cluster_communicator.get_master_config()
            )
    ...
    def start_ray_cluster(self, master_ip):
        if ray.__version__ >= "0.6.5":
            ...
            p = subprocess.Popen(
                "ray start --head --redis-port=6379 --no-ui --node-ip-address=%s"
                % master_ip,
                shell=True,
                stderr=subprocess.STDOUT,
            )
    ...
    def join_ray_cluster(self, master_ip, node_ip):
        p = subprocess.Popen(
            "ray start --redis-address=%s:6379 --node-ip-address=%s"
            % (master_ip, node_ip),
            shell=True,
            stderr=subprocess.STDOUT,
        )
        ...
        if p.poll() != 0:
            raise RuntimeError(
                "Could not join Ray server running at %s:6379" % master_ip
            )
    ...
    def copy_checkpoints_to_model_output(self):
        ...
        for source_path in latest_checkpoints:
            ...
            print(
                "Saved the checkpoint file %s as %s" % (source_path, destination_path)
            )
    ...
    def save_checkpoint_and_serving_model(
    ...
    def set_up_checkpoint(self, config=None):
        ...
        if ray.__version__ >= "0.6.5":
            if validation is not 1:
                raise RuntimeError(
                    "Failed to find .tune_metadata to restore checkpoint."
                )
        else:
            if validation is not 2:
                raise RuntimeError(
    ...
    def _checkpoint_dir_finder(self, current_dir=None):
        ...
        if len(current_dir_subfolders) > 1:
            ...
            return current_dir
        return self._checkpoint_dir_finder(
            os.path.join(current_dir, *current_dir_subfolders)
        )
    ...
    def launch(self):
        ...
        self.save_checkpoint_and_serving_model(
            algorithm=algo, env_string=env_string, config=config
        )
