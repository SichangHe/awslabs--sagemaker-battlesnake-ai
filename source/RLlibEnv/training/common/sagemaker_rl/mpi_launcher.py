...
class MPIMaster(object):
    ...
    def _wait_for_worker_nodes_to_start_sshd(
        self, hosts, interval=1, timeout_in_seconds=180
    ):
    ...
    def _build_mpi_command(self):
        ...
        print(
            "network interface name:" + interface_name + " " + str(self.instance_type)
        )
...
class MPIWorker(object):
    ...
    def __call__(self, env):
        ...
        print(
            "Worker node {} is waiting for MPI to start training process".format(
                current_host
            )
        )
...
class MPILauncher(object):
    ...
    def mpi_run(self):
        ...
        print(
            "MPI requested with process per hosts: {}".format(
                self._num_of_processes_per_host
            )
        )
        ...
        mpi_master = MPIMaster(
            env, self._num_of_processes_per_host, self._instance_type
        )
