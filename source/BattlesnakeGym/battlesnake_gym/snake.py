...
class Snake:
    ...
    def __init__(self, starting_position: tuple[int, int], map_size: tuple[int, int]):
        ...
        self._number_of_initial_body_stacking = (
            2  # At the start of the game, snakes of size 3 are stacked.
        )
    ...
    def make_from_list(cls, locations, health, map_size):
        ...
        if len(cls.locations) > 1:
            ...
            difference = (
                snake_head[0] - snake_2nd_body[0],
                snake_head[1] - snake_2nd_body[1],
            )
...
class Snakes:
    ...
    def get_snake_numbered_map(self, excluded_snakes=[]):
        ...
        """
        return np.sum(
            self.get_snake_depth_numbered_map(excluded_snakes=excluded_snakes), 2
        )
