...
def get_action(net, state, snake_id, turn_count, health, memory):
    ...
    previous_state_i = sort_states_for_snake_id(
        memory.state, snake_id + 1, one_versus_all=True
    )
    state_sequence = (
        mx.nd.array(np.stack([previous_state_i, state_i]), ctx=ctx)
        .transpose((0, 3, 1, 2))
        .expand_dims(0)
    )

    snake_id_sequence = mx.nd.array([snake_id] * sequence_length, ctx=ctx).expand_dims(
        0
    )
    turn_count_sequence = mx.nd.array(
        [memory.turn_count, turn_count], ctx=ctx
    ).expand_dims(0)
    snake_health_sequence = mx.nd.array(
        [memory.health[snake_id], health[snake_id]], ctx=ctx
    ).expand_dims(0)

    action = net(
        state_sequence, snake_id_sequence, turn_count_sequence, snake_health_sequence
    )
    """
...
def simulate(env, net, heuristics, number_of_snakes):
    ...
    while True:
        ...
        memory = Memory(
            state=state, turn_count=infos["current_turn"], health=infos["snake_health"]
        )
