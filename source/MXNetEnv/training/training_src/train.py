...
def run(seed, args):
    ...
    if args.writer:
        writer = SummaryWriter(
            "logs/{}-seed{}".format(args.run_name, seed), verbose=False
        )
    else:
        writer = None
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--gamma", type=float, default=0.95, help="discount factor (default: 0.95)"
    )
    parser.add_argument(
        "--tau", type=float, default=1e-3, help="soft update factor (default: 1e-3)"
    )
    parser.add_argument(
    ...
    parser.add_argument("--max_t", type=int, default=1000, help="Max t (default: 1k)")
    parser.add_argument(
        "--eps_start", type=float, default=1.0, help="Episilon start (default: 1.0)"
    )
    parser.add_argument(
        "--eps_end", type=float, default=0.01, help="Episilon end (default: 0.01)"
    )
    parser.add_argument(
        "--eps_decay", type=float, default=0.995, help="Episilon decay (default: 0.995)"
    )
    parser.add_argument("--warmup", type=float, default=0, help="Warmup (default: 0)")
    ...
    parser.add_argument(
        "--kernel_size", type=int, default=3, help="kernel size for the qnetwork"
    )
    parser.add_argument(
        "--repeat_size", type=int, default=3, help="Size to repeat input states"
    )
    parser.add_argument(
        "--activation_type",
        type=str,
    ...
    parser.add_argument(
        "--save_model_every", type=int, default=2000, help="Steps to save the model"
    )
    parser.add_argument("--model_dir", type=str, default=None)
    parser.add_argument(
        "--render_steps", type=int, default=1000, help="Steps to render (default: 1000)"
    )
    parser.add_argument(
        "--should_render",
        action="store_true",
    ...
    parser.add_argument(
        "--writer", action="store_true", help="should write to tensorboard"
    )
    parser.add_argument(
        "--print_progress",
        action="store_true",
