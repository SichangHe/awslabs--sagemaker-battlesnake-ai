...
from networks.qnetworks import QNetworkAttention, QNetworkConcat, QNetworkVision
...
class MultiAgentsCollection:
    ...
    def get_actions(self, state, dones, info, turn_count, eps):
        ...
        for i, agent in agents.items():
            ...
            action = agent.act(
                state_agent_i, i, turn_count, snake_health_i, episode, eps
            )
            actions[i] = action
    ...
    def step(
        self, state, actions, reward, next_state, dones, info, turn_count, should_learn
    ):
        for i, agent in agents.items():
            if i not in actions:
                continue
    ...
    def save(self, name, i_episode):
        if self.models_to_save == "all":
            agents[0].qnetwork_local.save_parameters(
                "{}/local-{}-e{}.params".format(self.model_dir, name, i_episode)
            )
            agents[0].qnetwork_local.export(
                "{}/local-{}-e{}".format(self.model_dir, name, i_episode)
            )
            agents[0].qnetwork_target.save_parameters(
                "{}/target-{}-e{}.params".format(self.model_dir, name, i_episode)
            )
            agents[0].qnetwork_target.export(
                "{}/target-{}-e{}".format(self.model_dir, name, i_episode)
            )
        else:
            agents[0].qnetwork_local.export("{}/local".format(self.model_dir))
...
class Agent:
    ...
    def __init__(
        ...
        self.schedule = mx.lr_scheduler.FactorScheduler(step=lr_step, factor=lr_factor)
        self.schedule.base_lr = lr_start
        adam_optimizer = mx.optimizer.Adam(
            learning_rate=lr_start, lr_scheduler=self.schedule
        )

        self.local_model_params = self.qnetwork_local.collect_params()
        self.target_model_params = self.qnetwork_target.collect_params()

        self.trainer = gluon.Trainer(self.local_model_params, optimizer=adam_optimizer)

        self.memory = ReplayBuffer(
            agent_id, action_size, buffer_size, self.batch_size, seed
        )

        self.loss_function = gluon.loss.L2Loss()
        self.done = False

    def act(self, state, snake_id, turn_count, snake_health, episode, eps=0.0):
        ...
        if random.random() > eps:
            ...
            with autograd.predict_mode():
                ...
                state_sequence = (
                    mx.nd.array(np.stack(state_sequence), ctx=ctx)
                    .transpose((0, 3, 1, 2))
                    .expand_dims(0)
                )
                turn_count_sequence = mx.nd.array(
                    np.stack(turn_count_sequence), ctx=ctx
                ).expand_dims(0)
                snake_health_sequence = mx.nd.array(
                    np.stack(snake_health_sequence), ctx=ctx
                ).expand_dims(0)

                snake_id_sequence = mx.nd.array(
                    np.array([snake_id] * self.sequence_length), ctx=ctx
                ).expand_dims(0)
            ...
            if self.qnetwork_target.take_additional_forward_arguments:
                Q_targets_next = (
                    self.qnetwork_target(
                        next_states, snake_id, turn_count, snake_health
                    )
                    .max(1)
                    .expand_dims(1)
                )
    ...
    def soft_update(self, tau):
        ...
        """
        for target_name, local_name in zip(
            self.target_model_params, self.local_model_params
        ):
    ...
    def sample_from_index(agent_id, sequence_length, index):
        ...
        for i in range(sequence_length):
            ...
            if episode_correct and turn_count_correct:
                ...
                next_states_i.insert(0, experience_i.next_state.transpose((2, 0, 1)))
    ...
    def sample(self, sequence_length):
        ...
        for index in indexes:
            (
                ...
                turn_count_i,
                snake_health_i,
            ) = ReplayBuffer.sample_from_index(self.agent_id, sequence_length, index)
        ...
        states = mx.nd.array(states, ctx=ctx).astype(np.float32, copy=False)
        actions = mx.nd.array(actions, ctx=ctx).astype(np.float32, copy=False)
        rewards = mx.nd.array(rewards, ctx=ctx).astype(np.float32, copy=False)
        next_states = mx.nd.array(next_states, ctx=ctx).astype(np.float32, copy=False)
        dones = mx.nd.array(dones, ctx=ctx).astype(np.float32, copy=False)
        snake_id = mx.nd.array(snake_id, ctx=ctx).astype(np.float32, copy=False)
        turn_count = mx.nd.array(turn_count, ctx=ctx).astype(np.float32, copy=False)
        snake_health = mx.nd.array(snake_health, ctx=ctx).astype(np.float32, copy=False)
