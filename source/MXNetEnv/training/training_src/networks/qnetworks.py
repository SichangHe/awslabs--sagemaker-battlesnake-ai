...
class QNetworkAttention(gluon.nn.HybridBlock):
    def __init__(
        ...
        self.conv_snake = gluon.nn.Conv2D(
            starting_channels, kernel_size=kernel_size, strides=2
        )
        ...
        self.conv_health = gluon.nn.Conv2D(
            starting_channels, kernel_size=kernel_size, strides=2
        )
        ...
        self.conv_turn = gluon.nn.Conv2D(
            starting_channels, kernel_size=kernel_size, strides=2
        )
        ...
        self.conv_predict = gluon.nn.Conv2D(
            starting_channels, kernel_size=kernel_size, strides=2
        )
        ...
        if self.sequence_length > 1:
            self.gru = gluon.rnn.GRU(
                number_of_hidden_states, num_layers=1, layout="NTC"
            )
    ...
    def hybrid_forward(
        ...
        resized_state_sequence = state_sequence.repeat(
            axis=3, repeats=self.repeat_size
        ).repeat(axis=4, repeats=self.repeat_size)
        new_state_sequence = resized_state_sequence.reshape((-3, -2))

        states_conv = self.conv(new_state_sequence)

        attentions = [
            (self.conv_snake, self.embedding_snake, snake_id_sequence, self.dS),
            (self.conv_turn, self.embedding_turn, turn_count_sequence, self.dT),
            (self.conv_health, self.embedding_health, snake_health_sequence, self.dH),
        ]
        ...
        outs_reshaped = F.reshape_like(
            outs, key_external, lhs_begin=1, lhs_end=2, rhs_begin=1, rhs_end=3
        )
        ...
        if self.sequence_length > 1:
            predict_reshaped = F.reshape_like(
                predict, state_sequence, lhs_begin=0, lhs_end=1, rhs_begin=0, rhs_end=2
            )
    ...
    def load_only_conv_layers(self, filename):
        ...
        layer_key_weight = [
            ...
            "conv2.weight",
            "conv_snake.weight",
            "embedding_snake.weight",
            "conv_health.weight",
            "embedding_health.weight",
            "conv_turn.weight",
            "embedding_turn.weight",
            "conv_predict.weight",
        ]
        layer_weight = [
            self.conv,
...
class QNetworkConcat(gluon.nn.HybridBlock):
    def __init__(
        ...
        with self.net.name_scope():
            ...
            for _ in range(number_of_dense_layers):
                self.net.add(
                    gluon.nn.Dense(number_of_hidden_states, activation=activation_type)
                )
    ...
    def hybrid_forward(
        ...
        if self.sequence_length == 1:
            ...
            concatenated = F.concat(
                state_dense,
                new_snake_id_embedding,
                new_turn_count_sequence,
                new_snake_health_sequence,
            )
            combined = self.combine_net(concatenated)
            states = F.reshape_like(
                combined, state_sequence, lhs_begin=0, lhs_end=1, rhs_begin=0, rhs_end=2
            )
            ts = self.gru(states)
            x = self.predict(ts)
