...
model_names = ["Model-11x11", "Model-15x15", "Model-19x19", "Model-7x7"]


def model_fn(model_dir):
    print(
        "model_fn model_dir={} glob={}".format(
            model_dir, glob.glob("{}/*".format(model_dir))
        )
    )
    ...
    for model_name in model_names:
        ...
        model = gluon.SymbolBlock.imports(
            symbol_name, ["data0", "data1", "data2", "data3"], params_name
        )
        print(
            "model_fn {} symbol={} params={}".format(
                model_name, symbol_name, params_name
            )
        )
...
def transform_fn(models, data, content_type, output_content_type):
    ...
    converted_action, _ = heuristics.run(
        heuristics_state,
        int(heuristics_id),
