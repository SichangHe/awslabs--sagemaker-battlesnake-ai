...
class ObservationToStateConverter:
    ...
    def _convert_to_state(self, game_state, game):
        ...
        state = np.zeros(
            (game.board_h + 2 * border_size, game.board_w + 2 * border_size, channels)
        )
        ...
        if self.border_option in ["1", "max"]:
            state = state - 1
            state[
                border_size : border_size + game.board_h,
                border_size : border_size + game.board_w,
                :,
            ] = 0
        ...
        for snake in game_state["board"]["snakes"]:
            ...
            if self.style == "one_versus_all":
                snake_channel = (
                    BattlesnakeGame.SNAKES_INDEX
                    if game.snake_to_ids[snake["id"]] != BattlesnakeGame.YOU_INDEX
                    else BattlesnakeGame.YOU_INDEX
                )
            else:
    ...
    def get_game_state(self, game_state):
        ...
        if game_id not in self.games:
            self.games[game_id] = {
                "game": BattlesnakeGame(game_state),
                "previous_state": None,
            }
